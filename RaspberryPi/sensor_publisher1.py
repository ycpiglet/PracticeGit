...
def button_callback(pin_Switch):
        global button, pub1, pub2, distance
        print("Button pushed!!")
        pub2.publish(distance)
        button += 1

def print_topic(data):
        print(data.data)

# ROS setting
rospy.init_node("topic_led")
sub = rospy.Subscriber("new_topic", String, print_topic)
pub1 = rospy.Publisher('counter', Int32, queue_size=10)
pub2 = rospy.Publisher("distance", String, queue_size=20)
rate = rospy.Rate(2)
...
while not rospy.is_shutdown():
    try:
        ...
        count += 1
        Distance = "Distance : %.2f cm"%distance
