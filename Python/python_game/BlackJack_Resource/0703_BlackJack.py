...
class BlackJack:                    # 클래스 BlackjJack
    def pressedB50(self):
        self.betMoney += 50
    ...
    def hitPlayer(self, n):
        ...
        self.player.addCard(newCard)        # Card 객체를 추가
        p = PhotoImage(file='cards/' + newCard.filename())      # Card 객체의 filename으로 이미지 생성
        self.LcardsPlayer.append(Label(self.window, image=p))       # 플레이어카드 이미지 라벨 리스트에 추가
        self.LcardsPlayer[self.player.inHand()-1].image = p         # 파이썬에서 라벨 이미지 레퍼런스 지정해줘야 이미지가 보임
        self.LcardsPlayer[self.player.inHand()-1].place(x=250+n*30, y=350)      # n : 위치변수 따라서 x 위치가 달라짐
    ...
    def hitDealerDown(self):    # 딜러 첫번째 카드 뒤집어서 생성
        ...
        self.LcardsDealer.append(Label(self.window, image=p))  # 플레이거 카드 이미지 라벨 리스트에 추가
        self.LcardsDealer[self.dealer.inHand() - 1].image = p  # 파이썬에서 라벨 이미지 레퍼런스 지정해줘야 이미지가 보임
    ...
    def hitDealer(self, n):
        ...
        self.deckN += 1  # 카드덱 인덱스 1 증가
        self.player.addCard(newCard)  # Card 객체를 추가
        p = PhotoImage(file='cards/' + newCard.filename())  # 뒤집어진 카드 이미지 생성
        self.LcardsDealer.append(Label(self.window, image=p))  # 플레이거 카드 이미지 라벨 리스트에 추가
        self.LcardsDealer[self.dealer.inHand() - 1].image = p  # 파이썬에서 라벨 이미지 레퍼런스 지정해줘야 이미지가 보임
        self.LcardsDealer[self.dealer.inHand() - 1].place(x=280+n*30, y=150)  # n : 위치변수 따라서 x 위치가 달라짐

    def checkWinner(self):
        p = PhotoImage(file='cards/' + self.dealer.cards[0].filename())     # 딜러의 Card 객체 리스트에서 0번째 Card 객체에서 filename 가져와서 이미지를 생성
    ...
    def pressedStay(self):          # 플레이어가 카드를 그만 받을 때 처리하는 함수
    ...
    def setupLable(self):           # 라벨 생성 함수
        ...
        self.LplayerPts = Label(text='', width=2, height=1, font=self.fontstyle2, bg='green', fg='white')  # 베팅머니 라벨
        self.LplayerPts.place(x=300, y=300)  # 절대배치
        self.LdealerPts = Label(text='', width=2, height=1, font=self.fontstyle2, bg='green', fg='white')  # 베팅머니 라벨
    ...
    def __init__(self):
        ...
        self.fontstyle = font.Font(self.window, size=24, weight='bold', family='Consoles')
        self.fontstyle2 = font.Font(self.window, size=16, weight='bold', family='Consoles')
        self.setupButton()          # 버튼 설정함수 호출
        self.setupLable()           # 라벨 설정함수 호출
        ...
        self.deckN = 0                      # 카드 덱은 (0..51) 숫자를 shuffle해서 가지고 있다
        
        self.window.mainloop()

BlackJack()         # BlackJack 객체 생성
