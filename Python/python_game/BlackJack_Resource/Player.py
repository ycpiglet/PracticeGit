class Player:           #클래스 Player (딜러와 플레이어 객체를 만들 때 사용)
    ...
    def inHand(self):           # 현재 갖고 있는 카드 개수를 반환 함수
        return self.N
    ...
    def value(self):            # Player 가 갖고 있는 카드들의 점수를 계산해서 반환
                                # 일단 모든 ACE 는 11로 계산해서 합산
                                # 만약 점수가 21이 넘어 가면 갖고 있는 ACE를 하나씩 1로 변경한다.
        total = 0               # 카드들의 합 점수
        ace = 0                 # ACE 카드의 개수 변수
        for i in range(self.N): # i=0,1,2,...N-1
            total += self.cards[i].getValue()   # i번째 카드 객체의 점수 (멤버함수 getValue() 호출)
